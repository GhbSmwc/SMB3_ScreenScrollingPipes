...
;so that you can multiple blocks with different variations (such as having some pipe caps that allow carrying sprites or allowing yoshi).
	...
	!Setting_SSP_Hijack_00EA18	= 1
		...
		; bottom corners.
		;
		; Make sure you avoid doing this: set this to 1, patch Fixes.asm, and then patch WJNB fix afterwards, because the WJNB fix will merely overwrite
	...
	!Setting_SSP_AllowCameraPanningWhenFrozen = 1
		...
		; and L/R scrolling when moving the camera, as handled at $00CDDD).
		; - 0 = no.
		; - 1 = yes.
		; Notes:
